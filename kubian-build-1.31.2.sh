...
wget https://pkgs.k8s.io/core:/stable:/v1.31/deb/Release.key -O - | gpg --batch --yes --dearmor --output /etc/apt/keyrings/kubernetes-apt-keyring.gpg
...
which helm
if [ $? != 0 ] ; then
  wget https://get.helm.sh/helm-v3.16.2-linux-amd64.tar.gz -O - | tar Cxzf /tmp - && cp /tmp/linux-amd64/helm /usr/local/bin/
fi
...
# cri-tools
cri-tools 1.31.1-1.1 amd64
# kubernetes
kubeadm 1.31.2-1.1 amd64
kubectl 1.31.2-1.1 amd64
kubelet 1.31.2-1.1 amd64
...
################################################################################
# cert-manager 1.16.1 -> https://artifacthub.io/packages/helm/cert-manager/cert-manager/1.16.1
...
quay.io/jetstack/cert-manager-acmesolver:v1.16.1
################################################################################
# https://github.com/kubernetes/ingress-nginx?tab=readme-ov-file#changelog
# ingress-nginx v1.11.3 -> https://github.com/kubernetes/ingress-nginx/blob/helm-chart-4.11.3/charts/ingress-nginx/values.yaml#L29
...
################################################################################
# calico v3.28.2 -> https://artifacthub.io/packages/helm/projectcalico/tigera-operator/3.28.2
...
################################################################################
# k8s 1.31.2 -> https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.31.md#container-images
...
# download helm v3.16.2 -> https://github.com/helm/helm/releases/tag/v3.16.2
if [ -f artefact/helm-v3.16.2-linux-amd64.tar.gz ] ; then
  echo "file exists artefact/helm-v3.16.2-linux-amd64.tar.gz" 
else
  wget https://get.helm.sh/helm-v3.16.2-linux-amd64.tar.gz -P artefact
fi
...
# download velero -> https://github.com/vmware-tanzu/velero/releases/tag/v1.14.1
if [ -f artefact/velero-v1.14.1-linux-amd64.tar.gz ] ; then
  echo "file exists artefact/velero-v1.14.1-linux-amd64.tar.gz" 
else
  wget https://github.com/vmware-tanzu/velero/releases/download/v1.14.1/velero-v1.14.1-linux-amd64.tar.gz -P artefact
fi
...
################################################################################
# install helm
tar Cxzf /tmp artefact/helm-v3.16.2-linux-amd64.tar.gz $SUPRESS_STDERR && cp /tmp/linux-amd64/helm /usr/local/bin/
...
################################################################################
# install velero
tar Cxzf /tmp artefact/velero-v1.14.1-linux-amd64.tar.gz $SUPRESS_STDERR && cp /tmp/velero-v1.14.1-linux-amd64/velero /usr/local/bin/
...
if [ \$CLUSTER = true ] ; then
  ################################################################################
  # install projectcalico tigera-operator v3.28.2
  gum spin --title "Install helm tigera-operator ..." -- helm upgrade --install tigera-operator helm/tigera-operator-v3.28.2.tgz \
    --create-namespace \
