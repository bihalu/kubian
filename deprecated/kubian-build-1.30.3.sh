...
wget https://pkgs.k8s.io/core:/stable:/v1.30/deb/Release.key -O - | gpg --batch --yes --dearmor --output /etc/apt/keyrings/kubernetes-apt-keyring.gpg
...
which helm
if [ $? != 0 ] ; then
  wget https://get.helm.sh/helm-v3.15.3-linux-amd64.tar.gz -O - | tar Cxzf /tmp - && cp /tmp/linux-amd64/helm /usr/local/bin/
fi
...
which yq
if [ $? != 0 ] ; then
  wget https://github.com/mikefarah/yq/releases/download/v4.44.3/yq_linux_amd64 -O /usr/local/bin/yq && chmod +x /usr/local/bin/yq
fi
...
# curl
curl 7.88.1-10+deb12u7 amd64
libcurl4:amd64 7.88.1-10+deb12u7 amd64
# ethtool
...
# cri-tools
cri-tools 1.30.1-1.1 amd64
# kubernetes
kubeadm 1.30.3-1.1 amd64
kubectl 1.30.3-1.1 amd64
kubelet 1.30.3-1.1 amd64
kubernetes-cni 1.4.0-1.1 amd64
# containerd
...
################################################################################
# openebs 3.10.0 -> https://artifacthub.io/packages/helm/openebs/openebs/3.10.0
...
################################################################################
# cert-manager 1.15.2 -> https://artifacthub.io/packages/helm/cert-manager/cert-manager/1.15.2
...
################################################################################
# ingress-nginx v1.11.1 -> https://github.com/kubernetes/ingress-nginx/blob/helm-chart-4.11.1/charts/ingress-nginx/values.yaml#L29
...
################################################################################
# calico v3.28.1 -> https://artifacthub.io/packages/helm/projectcalico/tigera-operator/3.28.1
quay.io/tigera/operator:v1.34.3
docker.io/calico/apiserver:v3.28.1
docker.io/calico/cni:v3.28.1
docker.io/calico/csi:v3.28.1
docker.io/calico/kube-controllers:v3.28.1
docker.io/calico/node-driver-registrar:v3.28.1
docker.io/calico/node:v3.28.1
docker.io/calico/pod2daemon-flexvol:v3.28.1
docker.io/calico/typha:v3.28.1
################################################################################
# k8s 1.30.3 -> https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.30.md#container-images
...
https://charts.jetstack.io jetstack cert-manager v1.15.2
https://kubernetes.github.io/ingress-nginx ingress-nginx ingress-nginx 4.11.1
https://projectcalico.docs.tigera.io/charts projectcalico tigera-operator v3.28.1
...
# download helm v3.15.3 -> https://github.com/helm/helm/releases/tag/v3.15.3
if [ -f artefact/helm-v3.15.3-linux-amd64.tar.gz ] ; then
  echo "file exists artefact/helm-v3.15.3-linux-amd64.tar.gz" 
else
  wget https://get.helm.sh/helm-v3.15.3-linux-amd64.tar.gz -P artefact
fi

# download k9s v0.32.5 -> https://github.com/derailed/k9s/releases/tag/v0.32.5
if [ -f artefact/k9s_Linux_amd64.tar.gz ] ; then
  echo "file exists artefact/k9s_Linux_amd64.tar.gz" 
else
  wget https://github.com/derailed/k9s/releases/download/v0.32.5/k9s_Linux_amd64.tar.gz -P artefact
fi

# download velero -> https://github.com/vmware-tanzu/velero/releases/tag/v1.13.2
if [ -f artefact/velero-v1.14.0-linux-amd64.tar.gz ] ; then
  echo "file exists artefact/velero-v1.14.0-linux-amd64.tar.gz" 
else
  wget https://github.com/vmware-tanzu/velero/releases/download/v1.14.0/velero-v1.14.0-linux-amd64.tar.gz -P artefact
fi
...
if [ -f artefact/yq ] ; then
  ...
  wget https://github.com/mikefarah/yq/releases/download/v4.44.3/yq_linux_amd64 -O artefact/yq
fi
...
################################################################################
# install helm
tar Cxzf /tmp artefact/helm-v3.15.3-linux-amd64.tar.gz $SUPRESS_STDERR && cp /tmp/linux-amd64/helm /usr/local/bin/
...
################################################################################
# install velero
tar Cxzf /tmp artefact/velero-v1.14.0-linux-amd64.tar.gz $SUPRESS_STDERR && cp /tmp/velero-v1.14.0-linux-amd64/velero /usr/local/bin/
...
if [ \$CLUSTER = true ] ; then
  ################################################################################
  # install projectcalico tigera-operator v3.28.1
  gum spin --title "Install helm tigera-operator ..." -- helm upgrade --install tigera-operator helm/tigera-operator-v3.28.1.tgz \
    --create-namespace \
...
if [ \$SINGLE = true ] ; then
  ...
  gum spin --title "Install helm ingress-nginx ..." -- helm upgrade --install ingress-nginx helm/ingress-nginx-4.11.1.tgz \
    --create-namespace \
  ...
  gum spin --title "Install helm cert-manager ..." -- helm upgrade --install cert-manager helm/cert-manager-v1.15.2.tgz \
    --create-namespace \
    --namespace cert-manager \
    --version v1.15.2 \
