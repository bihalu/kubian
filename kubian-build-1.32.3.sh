...
which helm
if [ $? != 0 ] ; then
  wget https://get.helm.sh/helm-v3.17.2-linux-amd64.tar.gz -O - | tar Cxzf /tmp - && cp /tmp/linux-amd64/helm /usr/local/bin/
fi
...
which yq
if [ $? != 0 ] ; then
  wget https://github.com/mikefarah/yq/releases/download/v4.45.1/yq_linux_amd64 -O /usr/local/bin/yq && chmod +x /usr/local/bin/yq
fi
...
which sops
if [ $? != 0 ] ; then
  wget https://github.com/getsops/sops/releases/download/v3.10.1/sops-v3.10.1.linux.amd64 -O /usr/local/bin/sops && chmod +x /usr/local/bin/sops
fi
...
# kubernetes
kubeadm 1.32.3-1.1 amd64
kubectl 1.32.3-1.1 amd64
kubelet 1.32.3-1.1 amd64
...
################################################################################
# cert-manager 1.17.1 -> https://artifacthub.io/packages/helm/cert-manager/cert-manager/1.17.1
...
# ingress-nginx v1.12.1 -> https://github.com/kubernetes/ingress-nginx/blob/helm-chart-4.12.1/charts/ingress-nginx/values.yaml#L29
...
################################################################################
# calico v3.29.3 -> https://artifacthub.io/packages/helm/projectcalico/tigera-operator/3.29.3
quay.io/tigera/operator:v1.36.7
docker.io/calico/apiserver:v3.29.3
docker.io/calico/cni:v3.29.3
docker.io/calico/csi:v3.29.3
docker.io/calico/kube-controllers:v3.29.3
docker.io/calico/node-driver-registrar:v3.29.3
docker.io/calico/node:v3.29.3
docker.io/calico/pod2daemon-flexvol:v3.29.3
docker.io/calico/typha:v3.29.3
################################################################################
# k8s 1.32.3 -> https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.32.md#container-images
...
docker.io/velero/velero:v1.15.2
...
https://charts.jetstack.io jetstack cert-manager v1.17.1
https://kubernetes.github.io/ingress-nginx ingress-nginx ingress-nginx 4.12.1
https://projectcalico.docs.tigera.io/charts projectcalico tigera-operator v3.29.3
...
# download helm v3.17.2 -> https://github.com/helm/helm/releases/tag/v3.17.2
if [ -f artefact/helm-v3.17.2-linux-amd64.tar.gz ] ; then
  echo "file exists artefact/helm-v3.17.2-linux-amd64.tar.gz"
else
  wget https://get.helm.sh/helm-v3.17.2-linux-amd64.tar.gz -P artefact
fi

# download k9s v0.40.10 -> https://github.com/derailed/k9s/releases/tag/v0.40.10
if [ -f artefact/k9s_Linux_amd64.tar.gz ] ; then
  echo "file exists artefact/k9s_Linux_amd64.tar.gz"
else
  wget https://github.com/derailed/k9s/releases/download/v0.40.10/k9s_Linux_amd64.tar.gz -P artefact
fi

# download velero -> https://github.com/vmware-tanzu/velero/releases/tag/v1.15.2
if [ -f artefact/velero-v1.15.2-linux-amd64.tar.gz ] ; then
  echo "file exists artefact/velero-v1.15.2-linux-amd64.tar.gz"
else
  wget https://github.com/vmware-tanzu/velero/releases/download/v1.15.2/velero-v1.15.2-linux-amd64.tar.gz -P artefact
fi
...
if [ -f artefact/yq ] ; then
  ...
  wget https://github.com/mikefarah/yq/releases/download/v4.45.1/yq_linux_amd64 -O artefact/yq
fi
...
if [ -f artefact/sops ] ; then
  ...
  wget https://github.com/getsops/sops/releases/download/v3.10.1/sops-v3.10.1.linux.amd64 -O artefact/sops
fi
...
################################################################################
# install helm
tar Cxzf /tmp artefact/helm-v3.17.2-linux-amd64.tar.gz $SUPRESS_STDERR && cp /tmp/linux-amd64/helm /usr/local/bin/
...
################################################################################
# install velero
tar Cxzf /tmp artefact/velero-v1.15.2-linux-amd64.tar.gz $SUPRESS_STDERR && cp /tmp/velero-v1.15.2-linux-amd64/velero /usr/local/bin/
...
if [ \$CLUSTER = true ] ; then
  ################################################################################
  # install projectcalico tigera-operator v3.29.3
  gum spin --title "Install helm tigera-operator ..." -- helm upgrade --install tigera-operator helm/tigera-operator-v3.29.3.tgz \
    --create-namespace \
...
if [ \$SINGLE = true ] ; then
  ...
  gum spin --title "Install helm ingress-nginx ..." -- helm upgrade --install ingress-nginx helm/ingress-nginx-4.12.1.tgz \
    --create-namespace \
  ...
  gum spin --title "Install helm cert-manager ..." -- helm upgrade --install cert-manager helm/cert-manager-v1.17.1.tgz \
    --create-namespace \
    --namespace cert-manager \
    --version v1.17.1 \
